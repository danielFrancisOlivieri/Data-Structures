...
//Function to remove each instance of an item from the list. The parameter
      //removeItem specifies the item to be removed.
      //Postcondition: If removeItem is found in the list,
      //    all instances of it are removed and the list's length is decremented to accomodate the fewer elements
...
void arrayListType<elemType>::removeAt(int location)
{
    ...
    if (location < 0 || location >= length)
        cerr << "The location of the item to be removed "
             << "is out of range" << endl;
    if (typeid(list[location]) == typeid(8))
        {
              insertEnd(list[location]);
        replaceAt(location, list[length-2]);
        replaceAt(length - 1, 0);
        replaceAt(length - 2, 0);
        }
    else
    {
        ...
        insertEnd(list[location]);
        replaceAt(location, list[length-2]);
        replaceAt(length - 1, listVar);
        replaceAt(length - 2, listVar);
length--;
